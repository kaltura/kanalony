...
DimensionDefinition(Dimensions.entry, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnCountryCityClstEntryRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnCountryCityClstEntryRow = {
        MinutelyAggPrtnCountryCityClstEntryRow(row.partnerId, row.country, row.city, row.day, row.metric, row.minute.plusHours(timezoneOffsetFromUtc), row.entryId, row.value)
      }
