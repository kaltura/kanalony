...
DimensionDefinition(Dimensions.hour, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : HourlyAggPrtnEntryCv3Row, timezoneOffsetFromUtc : Int) : HourlyAggPrtnEntryCv3Row = {
        HourlyAggPrtnEntryCv3Row(row.partnerId, row.entryId, row.customVar3, row.year, row.metric, row.hour.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.value)
      }
