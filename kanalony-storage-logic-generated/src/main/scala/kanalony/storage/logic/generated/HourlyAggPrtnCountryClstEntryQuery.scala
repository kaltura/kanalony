...
DimensionDefinition(Dimensions.entry, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : HourlyAggPrtnCountryClstEntryRow, timezoneOffsetFromUtc : Int) : HourlyAggPrtnCountryClstEntryRow = {
        HourlyAggPrtnCountryClstEntryRow(row.partnerId, row.country, row.month, row.metric, row.hour.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.entryId, row.value)
      }
