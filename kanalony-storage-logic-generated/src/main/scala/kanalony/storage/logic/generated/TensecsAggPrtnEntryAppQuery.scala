...
DimensionDefinition(Dimensions.tenSeconds, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : TensecsAggPrtnEntryAppRow, timezoneOffsetFromUtc : Int) : TensecsAggPrtnEntryAppRow = {
        TensecsAggPrtnEntryAppRow(row.partnerId, row.entryId, row.application, row.metric, row.day, row.tensecs.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.value)
      }
