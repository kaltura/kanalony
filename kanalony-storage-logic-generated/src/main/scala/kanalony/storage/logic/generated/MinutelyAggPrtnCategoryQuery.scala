...
DimensionDefinition(Dimensions.minute, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnCategoryRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnCategoryRow = {
        MinutelyAggPrtnCategoryRow(row.partnerId, row.category, row.metric, row.day, row.minute.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.value)
      }
