...
DimensionDefinition(Dimensions.minute, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnEntryCv1Cv2Cv3Row, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnEntryCv1Cv2Cv3Row = {
        MinutelyAggPrtnEntryCv1Cv2Cv3Row(row.partnerId, row.entryId, row.customVar1, row.customVar2, row.customVar3, row.metric, row.day, row.minute.plusHours(timezoneOffsetFromUtc), row.value)
      }
