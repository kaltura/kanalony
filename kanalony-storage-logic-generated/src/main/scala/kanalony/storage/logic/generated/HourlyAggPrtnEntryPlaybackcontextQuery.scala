...
DimensionDefinition(Dimensions.hour, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : HourlyAggPrtnEntryPlaybackcontextRow, timezoneOffsetFromUtc : Int) : HourlyAggPrtnEntryPlaybackcontextRow = {
        HourlyAggPrtnEntryPlaybackcontextRow(row.partnerId, row.entryId, row.playbackContext, row.metric, row.year, row.hour.plusHours(timezoneOffsetFromUtc), row.value)
      }
