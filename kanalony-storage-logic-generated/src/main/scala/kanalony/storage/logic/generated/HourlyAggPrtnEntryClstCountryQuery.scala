package kanalony.storage.logic.generated
    ...
    class HourlyAggPrtnEntryClstCountryQuery(accessor : IHourlyAggPrtnEntryClstCountryTableAccessor) extends QueryBase[HourlyAggPrtnEntryClstCountryQueryParams, HourlyAggPrtnEntryClstCountryRow] with IUserActivityQuery {
      private[logic] override def extractParams(params: QueryParams): HourlyAggPrtnEntryClstCountryQueryParams = {
        ...
        HourlyAggPrtnEntryClstCountryQueryParams(params.startUtc, params.endUtc, partner_id,entry_id, params.metrics.map(_.name))
      }
...
DimensionDefinition(Dimensions.country, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : HourlyAggPrtnEntryClstCountryRow, timezoneOffsetFromUtc : Int) : HourlyAggPrtnEntryClstCountryRow = {
        HourlyAggPrtnEntryClstCountryRow(row.partnerId, row.entryId, row.metric, row.year, row.hour.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.country, row.value)
      }
