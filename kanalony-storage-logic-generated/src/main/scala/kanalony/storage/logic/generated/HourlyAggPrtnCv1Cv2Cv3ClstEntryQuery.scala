...
DimensionDefinition(Dimensions.entry, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : HourlyAggPrtnCv1Cv2Cv3ClstEntryRow, timezoneOffsetFromUtc : Int) : HourlyAggPrtnCv1Cv2Cv3ClstEntryRow = {
        HourlyAggPrtnCv1Cv2Cv3ClstEntryRow(row.partnerId, row.customVar1, row.customVar2, row.customVar3, row.month, row.metric, row.hour.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.entryId, row.value)
      }
