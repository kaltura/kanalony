...
DimensionDefinition(Dimensions.playbackContext, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnEntryClstPlaybackcontextRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnEntryClstPlaybackcontextRow = {
        MinutelyAggPrtnEntryClstPlaybackcontextRow(row.partnerId, row.entryId, row.metric, row.day, row.minute.plusHours(timezoneOffsetFromUtc), row.playbackContext, row.value)
      }
