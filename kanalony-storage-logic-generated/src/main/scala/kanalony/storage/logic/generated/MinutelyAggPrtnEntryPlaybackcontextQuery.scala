...
DimensionDefinition(Dimensions.minute, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnEntryPlaybackcontextRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnEntryPlaybackcontextRow = {
        MinutelyAggPrtnEntryPlaybackcontextRow(row.partnerId, row.entryId, row.playbackContext, row.metric, row.day, row.minute.plusHours(timezoneOffsetFromUtc), row.value)
      }
