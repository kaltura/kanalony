...
DimensionDefinition(Dimensions.entry, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnCv1Cv2Cv3ClstEntryRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnCv1Cv2Cv3ClstEntryRow = {
        MinutelyAggPrtnCv1Cv2Cv3ClstEntryRow(row.partnerId, row.customVar1, row.customVar2, row.customVar3, row.day, row.metric, row.minute.plusHours(timezoneOffsetFromUtc), row.entryId, row.value)
      }
