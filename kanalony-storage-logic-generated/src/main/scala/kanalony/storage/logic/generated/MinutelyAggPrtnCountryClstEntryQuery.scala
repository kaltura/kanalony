...
DimensionDefinition(Dimensions.entry, new DimensionConstraintDeclaration(QueryConstraint.Range)))
      ...
      override private[logic] def updateTimezoneOffset(row : MinutelyAggPrtnCountryClstEntryRow, timezoneOffsetFromUtc : Int) : MinutelyAggPrtnCountryClstEntryRow = {
        MinutelyAggPrtnCountryClstEntryRow(row.partnerId, row.country, row.day, row.metric, row.minute.withZone(DateTimeZone.forOffsetHoursMinutes(timezoneOffsetFromUtc / 60, timezoneOffsetFromUtc % 60)), row.entryId, row.value)
      }
