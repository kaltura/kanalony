...
class DailyCountQueryTests extends DailyQueryTestsBase[DailyCountQuery] {
  it("Should aggregate data by day")({
    ...
    val params = QueryParams(List(createPartnerDimensionDefintion(Set(1)), createEntryDimensionDefintion(Set("1")), createDayDimensionDefintion()), List(Metrics.play), new DateTime(1), new DateTime(2))
    val expectedParams = QueryParams(List(createPartnerDimensionDefintion(Set(1)), createEntryDimensionDefintion(Set("1")), createHourDimensionDefintion()), List(Metrics.play), new DateTime(1), new DateTime(2))
